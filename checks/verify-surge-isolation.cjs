...
(async()=>{
 ...
 for(const baseline of [true,false]){
   ...
   if(baseline)await context.route('http://127.0.0.1:4318/**',r=>{const file=({'/':'index.html','/app.js':'app.js','/v4.js':'v4.js'})[new URL(r.request().url()).pathname];return file?r.fulfill({path:path.resolve('baselines/v195-before-surge-theme',file),contentType:file.endsWith('.js')?'text/javascript':'text/html'}):r.continue();});
   ...
   for(const theme of ['base','reading'])for(const mode of ['light','dark']){
 ...
 console.log('PASS: 8 unchanged baseline screenshots for base/reading homepage and workspace, light and dark.');
