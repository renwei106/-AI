...
(async () => {
  ...
  const [download] = await Promise.all([intro.waitForEvent('download'), intro.locator('.download').first().click()]); assert.equal(download.suggestedFilename(), 'shiyu-extension-0.1.0.zip'); assert.equal(await download.failure(), null); checks.push('ZIP download, responsive introduction and system light/dark themes');
