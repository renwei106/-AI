...
class PaymentStore {
  ...
  create({ userId, requestId, provider, plan, amount, baseExpiry = 0 }) {
    ...
    this.db.prepare(`INSERT INTO orders (id,user_id,request_id,provider,plan_id,plan_name,amount,days,status,created_at,expires_at,base_expiry) VALUES (?,?,?,?,?,?,?,?,'created',?,?,?)`)
      .run(id, userId, requestId, provider, plan.id, plan.name, amount, plan.days, now, now + 30 * 60_000, baseExpiry);
    return this.get(id);
  }
  ...
  paid(id, transactionId, paidAt) {
    ...
    try {
      ...
      if (order.status === 'paid') {
        ...
        const start = Math.max(paidAt, order.base_expiry, this.membership(order.user_id));
