(()=>{
 ...
 const readManual=()=>/(?:^|;\s*)shiyu-language=(zh-CN|en|ja)(?:;|$)/.exec(document.cookie)?.[1];
 ...
 function mount(){const languages=state.settings.languages.filter(l=>l.enabled);document.querySelectorAll('[data-shiyu-language]').forEach(el=>{if(languages.length<2)el.remove()});if(languages.length<2)return;
  for(const host of document.querySelectorAll('body>header .header-right,#world-page .world-header,#member-center .member-heading')){
   ...
   wrap.append(trigger,menu);const anchor=host.querySelector('[data-action="settings"],.member-mode-toggle,[data-member-close]');host.insertBefore(wrap,anchor||null);
  }
 }
 ...
 const syncLanguage=async()=>{if(document.querySelector('dialog[open] form'))return;if(readManual()!==manual){location.reload();return}try{const r=await fetch('/api/shiyu/i18n/public',{cache:'no-store'});if(!r.ok)return;const next=await r.json();if(JSON.stringify(next.settings)!==JSON.stringify(state.settings)&&!document.querySelector('dialog[open] form'))location.reload()}catch{}};
 addEventListener('focus',syncLanguage);
 document.addEventListener('visibilitychange',()=>{if(!document.hidden)void syncLanguage()});
