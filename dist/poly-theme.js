...
(() => {
  ...
  const dockBefore=dock;dock=function(){dockBefore();if(effective().theme!=='poly')return;const trigger=$('.dock-trigger');if(!trigger)return;const label=trigger.querySelector('.dock-label')?.outerHTML||'<span class="dock-label">我的空间</span>';trigger.innerHTML=`<span class="poly-dock-gem">${gem}</span>${label}`;};
